...
/**
 * global ostream method for TElapseTime object
 */
 ...
 * @return TFileStatus FILE_STATUS_OPENED or FILE_STATUS_CREATED if successful
 */
TDvbDb::TFileStatus  TDvbDb::CreateDbFile(std::string dbname)
{
 ...
 */
TDvbDb::TFileStatus  TDvbDb::OpenDb(int /*flags*/)
{
    TFileStatus  status = FILE_STATUS_ERROR;

    if(Sqlite3ppWrapper.connect_v2(DbFileName.c_str(), (SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE)) == SQLITE_OK)  // Create Db
    {
        status = FILE_STATUS_OPENED;
    }
    else
 ...
 * Constructor
 *
 * @param handle reference of TDvbDb object
 * @param commit or rollback boolean
 */
